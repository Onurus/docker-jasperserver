...
if [ -f "/.do_deploy_jasperserver" ]; then
    ...
    sed -i -e "s|^dbHost.*$|dbHost=$DB_HOST|g; s|# dbPort=5432|dbPort=$DB_PORT|g; s|^dbUsername.*$|dbUsername=$DB_USER|g; s|^dbPassword.*$|dbPassword=$DB_PASSWORD|g; s|# js.dbName=jasperserver|js.dbName=$DB_NAME|g" default_master.properties
